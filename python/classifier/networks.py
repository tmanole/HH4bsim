...
class GhostBatchNorm1d(nn.Module): #https://arxiv.org/pdf/1705.08741v2.pdf has what seem like typos in GBN definition. I've replaced the running mean and std rules with Adam-like updates.
    def __init__(self, features, ghost_batch_size=32, number_of_ghost_batches=32, nAveraging=1, eta=0.9, bias=True):
...
class dijetResNetBlock(nn.Module):
    def __init__(self, jetFeatures, dijetFeatures, useOthJets='', nOtherJetFeatures=4, device='cpu', layers=None, inputLayers=None):
...
class quadjetResNetBlock(nn.Module):
    def __init__(self, dijetFeatures, quadjetFeatures, useOthJets=False, device='cpu', layers=None, inputLayers=None):
...
class ResNet(nn.Module):
    def __init__(self, jetFeatures, dijetFeatures, quadjetFeatures, combinatoricFeatures, useOthJets='', device='cpu', nClasses=1):
        super(ResNet, self).__init__()
        self.debug = False
        self.nj = jetFeatures
        self.nd, self.nAd = dijetFeatures, 2 #total dijet features, engineered dijet features
        self.nq, self.nAq = quadjetFeatures, 2##6 #total quadjet features, engineered quadjet features
    ...
    def forward(self, j, o, d, q):
        ...
        if self.training:
            randomR = torch.tensor(np.random.uniform(0,2.0, 1), dtype=torch.float).to('cpu')

        # apply each of the symmetry transformations over which we will average
        j = j.view(4, n, self.nj, 12) ### TODO 12/9)
