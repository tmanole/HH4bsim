...
parser.add_argument('-t', '--target', default='../../events/MG3/PtEtaPhi/CR4b.npy', type=str, help='Path to second array, in .npy format.')
parser.add_argument('-is', '--indsource', default='I_nn_MG3_CR3b_halfstep.npy', type=str, help='Output file path.')
parser.add_argument('-it', '--indtarget', default='I_nn_MG3_CR4b_halfstep.npy', type=str, help='Output file path.')
parser.add_argument('-Nl', '--Nlow', default=0, type=int, help='Output file path.')
parser.add_argument('-Nh', '--Nhigh', default=1, type=int, help='Output file path.')
parser.add_argument('-o', '--output', default='MG3_nn_blocks_halfstep/', type=str, help='Output file path.')
...
print("Source size: ", source.shape, target.shape)

output = args.output

I_CR = np.load(args.indtarget)
I_SR = np.load(args.indsource)

print("I shape : ", I_CR.shape, I_SR.shape)
print("I range: ",  np.max(I_CR), np.max(I_SR))
N = I_CR.shape[1]
print(N)
N_low = args.Nlow
N_high = args.Nhigh
